...
TrieNonLeafNode::TrieNonLeafNode(char ch){
    ...
    letters = new char[2];
    if(ptrs == nullptr || letters == nullptr){
        cerr << "Out of Memory3.\n";
        exit(1);
    }
    leaf = false;
    ...
    while(true){
        if(p->leaf){
            ...
            if(p->endOfWord) return true;
             ...
             return false;
        }else if((pos = position(p, *word)) != notfound && p->ptrs[pos] != nullptr){
            p = p->ptrs[pos];
            word++;
        }else return false;
    }
...
void Trie::addCell(char ch, TrieNonLeafNode *p, int stop){
    ...
    if(p->letters == nullptr || p->ptrs == nullptr){
        cerr << "Out of Memory1.\n";
        exit(1);
    }
    ...
    while (true) {
        if(*word == '\0'){
            ...
            if(pos == notfound){
                ...
                if(strlen(lf->word) > offset+1) s = lf->word+offset+1;
                else s = "";
